...
class UserController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($this->request->isPost) {
            if ($model->load($this->request->post())) {
                $plain_password = $this->request->post('User')['password'];
    ...
    public function beforeAction($action)
    {
        if (!Yii::$app->user->isGuest) {
            ...
            //echo '<pre>'; print_r($action->id); exit;
            if ($user->user_type !== 1 && in_array($action->id, ['delete', 'create', 'block', 'unblock'])) {
            ...
            if ($user->user_type === 3 && $action->id !== 'view' && $action->id !== 'update') {
